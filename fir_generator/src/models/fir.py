...
class ComplainantIDDocument(BaseModel):
    id_type: str = Field(alias="idType")
    id_number: str = Field(alias="idNumber")

class InquestReport(BaseModel):
    uidb_number: str = Field(alias="uidbNumber")
...
class FIRDataPayload(BaseModel):
    ...
    complainant_ids: List[ComplainantIDDocument] = []
    inquest_reports: List[InquestReport] = []
