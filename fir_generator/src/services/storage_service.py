...
class SupabaseStorageService:
    ...
    def update_fir_report_url(self, fir_id: str, pdf_url: str):
        ...
        """
        id_clean = fir_id.strip()
        print(f"🔄 Updating DB for FIR {id_clean} with URL: {pdf_url[:50]}...")
        try:
            res = self.supabase.table("fir_reports").update({"pdf_url": pdf_url}).eq("id", id_clean).execute()
            if not res.data:
                print(f"⚠️ Warning: No rows updated for FIR ID {id_clean}. Check if ID exists.")
            else:
                print(f"✅ DB Update successful for FIR {id_clean}.")
